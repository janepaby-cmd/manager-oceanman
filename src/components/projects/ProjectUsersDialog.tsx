  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("projectUsers")}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {assignedUsers.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">{t("noUsersAssigned")}</p>
          ) : (
            assignedUsers.map((u) => (
              <div key={u.id} className="flex items-center justify-between p-2 border rounded-md">
                <div>
                  <p className="text-sm font-medium">{u.profile?.full_name || t("common:user")}</p>
